...
export function populateWebsiteList(ul: HTMLUListElement, timeData: TimeData, monitored:string[]): void {
    ul.innerHTML = "";
    ...
    const pairs = monitored.map(site => {
        let total = 0;
        if (timeData[site]) {
            for(const date in timeData[site]) total += timeData[site][date].time;
        }
...
export async function renderCustomWebsiteTable(tbody: HTMLTableSectionElement): Promise<void> {
    tbody.innerHTML = "";
    const result = await chrome.storage.local.get(["TimeWastingDomains"]);
    const websites = (result.TimeWastingDomains as string[]) || [];
